pipeline {
    ...
    environment{
        ...
        CFN_TEMPLATE="phonebook-docker-swarm-cfn-template.yaml"
        CFN_KEYPAIR="deniz-keypair"
    ...
    stages {
        ...
        stage('Deploying the Application'){
            ...
            steps {
                ...
                sleep(10)
                sh 'mssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no --region ${AWS_REGION} ${MASTER_INSTANCE_ID} docker stack deploy --with-registry-auth -c ${HOME_FOLDER}/${GIT_FOLDER}/docker-compose.yaml ${APP_NAME}'
            }
        }
    }
    post {
